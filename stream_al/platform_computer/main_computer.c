...
{ 
#include "../stream_graph/graph_0_bin.txt"
};
...
/**
  ...
  @par              initialize a Graph interpreter  instance : share the address of the graph and
                    ...
                    Then start an infinite loop for the Graph interpreter processing.
  @remark
 */
int main(void)
{
...
#define STREAM_NB_INSTANCE 1
	...
	arm_graph_interpreter (STREAM_RESET, &instance, (uint32_t *)graph_input);

    /* run the graph */
	for (int i = 0; i < 1000000000L; i++)
    {   arm_graph_interpreter (STREAM_RUN, &instance, 0);
    }  

    arm_graph_interpreter (STREAM_STOP, &instance, 0);
}
