...
intptr_t pack2linaddr_int(uint32_t x, uint8_t ** LL)
{
    ...
    {   uint8_t *long_base;                                     
        ...
        long_base = LL[RD(x,ADDR_OFFSET_FMT0)];                       
...
void * pack2linaddr_ptr(uint32_t data, uint8_t ** long_offset)
{
...
{
    uint32_t i, size, offset, *table, *graph_src, *graph_dst, *ret;
    ...
    {   
        ret = graph_dst = pack2linaddr_ptr(table[SECTION_ADDR], S->long_offset);
...
void platform_init_stream_instance(arm_stream_instance_t *S)
{
    ...
    S->nb_graph_io = graph_input[GRAPH_HEADER_NBWORDS + GRAPH_PIO_GRAPH *2 + SECTION_SIZE] / STREAM_IOFMT_SIZE_W32; // number of graph IOs
