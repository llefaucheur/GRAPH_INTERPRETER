...
% Function to test the routine lagrange_new 
 ...
 x = 0:1.8:10;           % The interpolation points

%L=3;
%xk = (randn(1,L) ); xk = sort(xk)
%yk = (randn(1,L) )
%x = xk(1)-0.5: 0.1: xk(L)+0.5;
...
plot(x,y,'g');   hold on;
stem(xk,yk,'k-','LineWidth',1);    grid on;  title('   Points to be interpolated and the interpolated function');
xlabel('x');     ylabel('y');
