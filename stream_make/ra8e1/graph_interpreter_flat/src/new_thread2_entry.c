...
*/
#define STREAM_CURRENT_INSTANCE 2
#define STREAM_NB_INSTANCE 3

    /* GRAPH INSTANCE */
    extern arm_stream_instance_t instance[STREAM_NB_INSTANCE];
...
void new_thread2_entry(void *pvParameters)
{
    ...
    if (f_init_made == 0)
    {
    	vTaskPrioritySet( new_thread2, tskIDLE_PRIORITY + 3 );
		...
		instance[STREAM_CURRENT_INSTANCE].scheduler_control =
			PACK_STREAM_PARAM(
				...
				STREAM_SECONDARY_INSTANCE,          // this interpreter instance is NOT the main one 
