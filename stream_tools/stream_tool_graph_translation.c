...
#define GRAPH_ALL_MANIFESTS "../../../stream_tools/stream_tools_files_computer.txt"
...
void main(void)
{
    ...
    */
    {   FILE* ptf_graph_all_files;
        ...
        idx = 0;
        while (1 == fread(&(all_files[idx++]), 1, 1, ptf_graph_all_files));
        arm_stream_read_manifests(&platform, all_nodes, all_files);
        fclose(ptf_graph_all_files);
    }
    /*
        STEP 2: convert a Generic GRAPH file to a Platform-specific graph using 
    ...
    */
    {   FILE *ptf_ggraph, *ptf_pgraph;
        ...
        while (1 == fread(&(ggraph[idx++]), 1, 1, ptf_ggraph));
    ...
    /* 
        STEP 3 : convert PGRAPH file to a graph ready for an optional 
        memory map manual optimization ("gmap"), do byteCodes assembly,
        Buffer and SWC instances are provided with no physical address
    */
    {   FILE * ptf_gmap, * ptf_ggraph;
        uint32_t idx;
        ...
        arm_stream_ggraph2pgraph(&platform, all_nodes, ggraph, idx, ptf_gmap);
    ...
    */
    {   FILE* ptf_graph_omap, * ptf_graph_bin;
        ...
        while (1 == fread(&(pgraph[idx++]), 1, 1, ptf_graph_omap));
