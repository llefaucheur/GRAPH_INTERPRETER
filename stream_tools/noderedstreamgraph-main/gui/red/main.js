...
var RED = (function() {
	...
	function save(force) {
		RED.storage.update();
		// TODO Liam Replace with checkbox
		...
		if (RED.nodes.hasIO()) {
			...
			// TODO Liam Investigate why graph tool malfunctions when this compatability feature is removed
			var metaData = $.parseJSON($("script[data-container-name|='InputOutputCompatibilityMetadata']").html());
			requirements = metaData["requirements"];


			// #ECPS Create empty string which will be the YML format graph 
			// #ECPS Note YML does not use TABS, always use spaces
			var yml = "";
			if (export_as_yaml){
				yml += "version: " + "'1.0'" + "\n";
				yml += "graph:" + "\n";
			...
			for (var i=0; i<nns.length; i++) {
				...
				if (node && (node.outputs > 0 || node._def.inputs > 0)) {
					...
					// TODO Liam When graph is finalised create an array of field types to iterate through
					...
					if (export_as_yaml){
						...
						// TODO Liam: Investigate loop parameters type.length/n.id.length as they seem arbitrary
						yml += n.type + " ";
			...
			var cordcount = 1; // TODO Liam remove if un-used
			yml += "  arcs:\n";
			for (var i=0; i<nns.length; i++) {
				var n = nns[i];
				// TODO Add if n.args?
	...
	$(function() {
		...
		if (!server) {
			// TODO Liam Investigate why graph tool malfunctions when this compaability feature is removed
			var metaData = $.parseJSON($("script[data-container-name|='InputOutputCompatibilityMetadata']").html());
			requirements = metaData["requirements"];
